...
export class CameraService {
  ...
  create_camera(payload: ICreate_Camera) {
    return this.http.post<String>(server + "camera/create", payload);
  }
  ...
  getOne(primary_user: number, id_indicator: number){
    return this.http.get<ICamera_Recive>(server + `camera/getOne?primary_user=${primary_user}&id_indicator=${id_indicator}`);
  }
