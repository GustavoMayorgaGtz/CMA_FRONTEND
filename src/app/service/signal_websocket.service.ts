...
export class SignalService {
  ...
  ProduccionTotal(id: number) {
    ...
    // });
    this.socket.emit('joinGroup', { group: 'monitoring' }); // Unirse al grupo
  ...
  Alerta_TiempoParo(id: number) {
    this.socket.emit('joinGroup', { group: 'alert' }); // Unirse al grupo
    this.socket.on(`S${id}_Alert`, (data: string) => {
